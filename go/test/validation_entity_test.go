...
func TestValidationEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set BLUEFINDECRYPTX_TEST_VALIDATION_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		validationRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath([]any{"new", "validation"}, setup.data), "validation_ref01"))

		validationRef01DataResult, err := validationRef01Ent.Create(validationRef01Data, nil)
		if err != nil {
			t.Fatalf("create failed: %v", err)
		}
		validationRef01Data = core.ToMapAny(validationRef01DataResult)
		if validationRef01Data == nil {
...
func validationBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("BLUEFINDECRYPTX_TEST_VALIDATION_ENTID")
	...
	idmapResolved := core.ToMapAny(env["BLUEFINDECRYPTX_TEST_VALIDATION_ENTID"])
