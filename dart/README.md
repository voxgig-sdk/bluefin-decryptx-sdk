...
The Dart SDK for the BluefinDecryptx API — an entity-oriented client following idiomatic Dart conventions.
...
[Releases](https://github.com/voxgig-sdk/bluefin-decryptx-sdk/releases)) in your `pubspec.yaml`:

```yaml
dependencies:
  bluefin_decryptx_sdk:
    git:
      url: https://github.com/voxgig-sdk/bluefin-decryptx-sdk
      path: dart
      ref: dart/v0.0.1
```
...
final created = await client.Decrypt().create({'decryption_parameter': <String, dynamic>{}, 'encrypted': <dynamic>[], 'has_cc_data': true, 'message_id': 'example_message_id', 'name': 'example_name', 'reference': 'example_reference', 'serial': 'example_serial', 'success': true});
...
final decrypt = await client.Decrypt().list();
...
```bash
export BLUEFIN_DECRYPTX_TEST_LIVE=TRUE
export BLUEFIN_DECRYPTX_APIKEY=<your-key>
```
...
```dart
final client = BluefinDecryptxSDK.test(testopts, sdkopts);
```
...
| `Decrypt` | `([entopts]) -> DecryptEntity` | Create a Decrypt entity instance. |
| `DecryptExt` | `([entopts]) -> DecryptExtEntity` | Create a DecryptExt entity instance. |
| `Validation` | `([entopts]) -> ValidationEntity` | Create a Validation entity instance. |
...
Create an instance: `final decrypt = client.Decrypt();`
...
| `client_id` | `String` |  |
| `code` | `int` |  |
| `decrypted` | `List<dynamic>` |  |
| `decryption_parameter` | `Map<String, dynamic>` |  |
| `encrypted` | `List<dynamic>` |  |
| `failed` | `bool` |  |
| `firmware` | `String` |  |
| `has_cc_data` | `bool` |  |
| `message` | `String` |  |
...
```dart
final decrypt = await client.Decrypt().create({
...
Create an instance: `final decrypt_ext = client.DecryptExt();`
...
```dart
final decrypt_ext = await client.DecryptExt().create({
  'encrypted': 1,  // num
...
Create an instance: `final validation = client.Validation();`
...
```dart
final validation = await client.Validation().create({
  'message_id': 'example_message_id',  // String
  'reference': 'example_reference',  // String
  'serial': 'example_serial',  // String
  'success': true,  // bool
...
`BluefinDecryptxTypes.dart` provides a class per entity plus per-op request/match
classes (e.g. `BluefinDecryptx.fromMap(entity.data())` and `model.toMap()`), so you
...
dart/
├── lib/
│   ├── BluefinDecryptxSDK.dart          -- Main SDK library (exported entry point)
│   ├── BluefinDecryptxTypes.dart        -- Typed entity + request/match models
│   ├── BluefinDecryptxEntityBase.dart   -- Base class for entities
│   ├── BluefinDecryptxError.dart        -- SDK error type
...
The main library (`BluefinDecryptxSDK.dart`) re-exports the SDK class, the typed
