...
void tests() {
  describe('DecryptExtEntity', () {
    ...
    test('stream', (t) async {
      ...
      final plainent = plainsdk.DecryptExt();
      final seen2 = [];
    ...
    test('basic', (t) async {
      ...
      for (final op in ['create']) {
        if (maybeSkipControl(t, 'entityOp', 'decrypt_ext.' + op, live)) {
          return;
        }
      }
      ...
      if (true == setup['syntheticOnly']) {
        t.skip('live entity test uses synthetic IDs from fixture — set BLUEFIN_DECRYPTX_TEST_DECRYPT_EXT_ENTID JSON to run live');
        return;
      }
      ...
      // CREATE
      final decrypt_ext_ref01_ent = client.DecryptExt();
      dynamic decrypt_ext_ref01_data = setup['data']['new']['decrypt_ext']['decrypt_ext_ref01'];

      decrypt_ext_ref01_data = await decrypt_ext_ref01_ent.create(decrypt_ext_ref01_data);
      ok(null != decrypt_ext_ref01_data);
...
Map<String, dynamic> basicSetup([dynamic extra]) {
  ...
  dynamic idmap = transform(
      <dynamic>['decrypt_ext01', 'decrypt_ext02', 'decrypt_ext03'],
      <String, dynamic>{
        '`\$PACK`': <dynamic>[
          '',
          <String, dynamic>{
  ...
  final idmapEnvVal =
      Platform.environment['BLUEFIN_DECRYPTX_TEST_DECRYPT_EXT_ENTID'];
