...
package BluefinDecryptxContext;
...
sub new {
  ...
  my $gcp = \&BluefinDecryptxHelpers::get_ctx_prop;
  ...
  $self->{ctrl} = BluefinDecryptxControl->new();
  ...
  $self->{data} = BluefinDecryptxHelpers::to_map($gcp->($ctxmap, 'data')) || {};
  $self->{reqdata} = BluefinDecryptxHelpers::to_map($gcp->($ctxmap, 'reqdata')) || {};
  $self->{match} = BluefinDecryptxHelpers::to_map($gcp->($ctxmap, 'match')) || {};
  $self->{reqmatch} = BluefinDecryptxHelpers::to_map($gcp->($ctxmap, 'reqmatch')) || {};

  my $pt = $gcp->($ctxmap, 'point');
  $self->{point} = Voxgig::Struct::ismap($pt) ? $pt : ($basectx ? $basectx->{point} : undef);

  my $sp = $gcp->($ctxmap, 'spec');
  $self->{spec} = (Scalar::Util::blessed($sp) && $sp->isa('BluefinDecryptxSpec'))
    ? $sp : ($basectx ? $basectx->{spec} : undef);

  my $r = $gcp->($ctxmap, 'result');
  $self->{result} = (Scalar::Util::blessed($r) && $r->isa('BluefinDecryptxResult'))
    ? $r : ($basectx ? $basectx->{result} : undef);

  my $rp = $gcp->($ctxmap, 'response');
  $self->{response} = (Scalar::Util::blessed($rp) && $rp->isa('BluefinDecryptxResponse'))
...
sub resolve_op {
  ...
  return BluefinDecryptxOperation->new({}) if '' eq $opname;

  my $opcfg = BluefinDecryptxHelpers::gpath($self->{config}, "entity.$entname.op.$opname");

  my $input = ($opname eq 'update' || $opname eq 'create') ? 'data' : 'match';

  my $points = [];
  if (Voxgig::Struct::ismap($opcfg)) {
    my $t = BluefinDecryptxHelpers::gp($opcfg, 'points');
    $points = $t if Voxgig::Struct::islist($t);
  }

  my $op = BluefinDecryptxOperation->new({
    entity => $entname,
    name   => $opname,
    input  => $input,
    points => $points,
...
sub make_error {
  my ($self, $code, $msg) = @_;
  return BluefinDecryptxError->new($code, $msg, $self);
}
