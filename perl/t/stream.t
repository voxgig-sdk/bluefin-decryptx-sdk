...
sub has_feature {
  my ($name) = @_;
  my $f = BluefinDecryptxConfig::make_config()->{feature};
...
sub find_list_entity {
  ...
  my $entities = BluefinDecryptxHelpers::to_map(
    BluefinDecryptxHelpers::gp($config, 'entity')) || {};
  for my $name (sort keys %$entities) {
    my $points = BluefinDecryptxHelpers::gpath($config, "entity.$name.op.list.points");
    next unless Voxgig::Struct::islist($points);
    for my $p (@$points) {
      my $params = BluefinDecryptxHelpers::gpath($p, 'args.params');
      my $reqd = Voxgig::Struct::islist($params)
        ? scalar(grep { BluefinDecryptxHelpers::is_true($_->{reqd}) } @$params)
        : 0;
...
SKIP: {
  ...
  {
    my $sdk = BluefinDecryptxSDK->test({ 'entity' => $seed }, undef);
  ...
  if (has_feature('streaming')) {
    {
      my $sdk = BluefinDecryptxSDK->test(
        { 'entity' => $seed },
        { 'feature' => { 'streaming' => {
