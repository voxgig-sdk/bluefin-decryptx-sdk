...
package BluefinDecryptxTestRunner;
...
sub env_override {
  my ($m) = @_;
  my $live = getenv('BLUEFINDECRYPTX_TEST_LIVE');
  my $override = getenv('BLUEFINDECRYPTX_TEST_OVERRIDE');
  ...
  my $explain = getenv('BLUEFINDECRYPTX_TEST_EXPLAIN');
  $m->{'BLUEFINDECRYPTX_TEST_EXPLAIN'} = $explain if defined $explain && '' ne $explain;
...
sub is_control_skipped {
  ...
  my $skip = BluefinDecryptxHelpers::gpath($ctrl, "test.skip.$mode") || {};
  my $items = $skip->{$kind} || [];
...
sub live_delay_ms {
  my $ctrl = load_test_control();
  my $v = BluefinDecryptxHelpers::gpath($ctrl, 'test.live.delayMs');
