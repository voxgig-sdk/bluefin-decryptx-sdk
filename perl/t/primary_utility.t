...

{
  ...
  our @ISA = ('BluefinDecryptxBaseFeature');

  sub new {
    my ($class, $fn) = @_;
    my $self = BluefinDecryptxBaseFeature::new($class);
    $self->{hook_fn} = $fn;
    return $self;
  }
...
sub make_ctx_from_map {
  ...
  my $ctx = BluefinDecryptxContext->new($ctxmap, undef);
  ...
  if (Voxgig::Struct::ismap($ctxmap->{response})) {
    ...
    $ctx->{response} = BluefinDecryptxResponse->new($resp_map);
    if (BluefinDecryptxHelpers::rb_truthy($resp_map->{body})) {
...
sub err_from_map {
  ...
  return BluefinDecryptxError->new($code, $msg);
}
...

{
  ...
  my $feature = BluefinDecryptxBaseFeature->new;
...
{
  my $hook_client = BluefinDecryptxSDK->test(undef, undef);
...
{
  my $init_client = BluefinDecryptxSDK->test(undef, undef);
...
{
  my $calls = [];
  my $live_client = BluefinDecryptxSDK->new({
    'system' => {
      'fetch' => sub {
        my ($url, $fetchdef) = @_;
...
{
  my $blocked_client = BluefinDecryptxSDK->new({
    'system' => {
...
{
  my $ctx = make_test_full_ctx($client, $utility);
  $ctx->{spec} = BluefinDecryptxSpec->new({ 'step' => 'start' });
  $ctx->{result} = undef;
  my (undef, $err) = $utility->{make_result}->($ctx);
...

{
  ...
  my $spec_client = BluefinDecryptxSDK->test(undef, $setup_opts);
...
runset('makeUrl.basic', get_spec($PRIMARY, 'makeUrl', 'basic'), sub {
  ...
  my $ctxmap = $entry->{ctx} || {};
  my $ctx = make_ctx_from_map($ctxmap, $client, $utility);
  $ctx->{result} = BluefinDecryptxResult->new({}) unless $ctx->{result};
...
runset('operator.basic', get_spec($PRIMARY, 'operator', 'basic'), sub {
  my ($entry) = @_;
  my $in_val = $entry->{in} || {};
  my $op = BluefinDecryptxOperation->new($in_val);
  return {
    'entity' => $op->{entity},
    'name' => $op->{name},
    'input' => $op->{input},
...

{
  ...
  my $auth_client = BluefinDecryptxSDK->test(undef, $setup_opts);
