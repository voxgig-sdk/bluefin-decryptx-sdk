...
$BluefinDecryptxUtility::REGISTRAR = sub {
  my ($u) = @_;
  for my $k (keys %BluefinDecryptxUtilities::REGISTRY) {
    $u->{$k} = $BluefinDecryptxUtilities::REGISTRY{$k};
  }
  # The vendored struct utility, reachable as $utility->{struct}{<fn>}.
  $u->{struct} = BluefinDecryptxHelpers::struct_facade();
};
