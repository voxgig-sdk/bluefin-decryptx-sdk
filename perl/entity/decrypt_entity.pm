# BluefinDecryptx SDK Decrypt entity
...
sub new {
  ...
  elsif (BluefinDecryptxHelpers::is_false($entopts->{active})) {
    # keep false
  }
  else {
    $entopts->{active} = Voxgig::Struct::JTRUE();
...
sub make {
  ...
  return DecryptEntity->new($self->{_client}, $opts);
}

sub data_set {
  my ($self, $args) = @_;
  if ($args) {
    $self->{_data} = BluefinDecryptxHelpers::to_map(Voxgig::Struct::clone($args)) || {};
    $self->{_utility}{feature_hook}->($self->{_entctx}, 'SetData');
  }
  return;
}

# Returns the current Decrypt data (hashref).
sub data_get {
  my ($self) = @_;
...
sub match_set {
  my ($self, $args) = @_;
  if ($args) {
    $self->{_match} = BluefinDecryptxHelpers::to_map(Voxgig::Struct::clone($args)) || {};
    $self->{_utility}{feature_hook}->($self->{_entctx}, 'SetMatch');
  }
  return;
}

# Returns the current match filter (any subset of Decrypt fields).
sub match_get {
  my ($self) = @_;
...
# List Decrypt items matching the given filter.
#
# reqmatch: match filter hashref (any subset of Decrypt fields;
# DecryptListMatch shape); defaults to undef, treated as an empty match
# that lists all. ctrl: optional per-call control.
# Returns the matching Decrypt items as an arrayref; dies with
# BluefinDecryptxError on failure.
sub list {
...
# Create a new Decrypt.
#
# reqdata: body data hashref (DecryptCreateData shape). ctrl: optional
# per-call control. Returns the created Decrypt data (hashref); dies
# with BluefinDecryptxError on failure.
sub create {
  ...
  return $self->_run_op($ctx, sub {
    ...
    if ($result) {
      if ($result->{resdata}) {
        $self->{_data} = BluefinDecryptxHelpers::to_map(
...
sub stream {
  ...
  my $utility = $self->{_utility};
  $callopts = BluefinDecryptxHelpers::to_map($callopts) || {};
  my $signal = BluefinDecryptxHelpers::gp($callopts, 'signal');
  my $ctrl = BluefinDecryptxHelpers::to_map(
    BluefinDecryptxHelpers::gp($callopts, 'ctrl')) || {};
  ...
  my $ctx = $utility->{make_context}->({
    ...
    'match' => $self->{_match},
    'data' => $self->{_data},
    %{ BluefinDecryptxHelpers::to_map($args) || {} },
  }, $self->{_entctx});

  # Outbound: expose an async-iterable/list payload so the request builder /
  # transport can stream it as the request body.
  my $body = BluefinDecryptxHelpers::gp($callopts, 'body');
  if (defined $body) {
    my $reqdata = BluefinDecryptxHelpers::to_map($ctx->{reqdata}) || {};
    $reqdata->{'body$'} = $body;
    $ctx->{reqdata} = $reqdata;
    $ctx->{stream_out} = $body;
  }
