# BluefinDecryptx SDK DecryptExt entity
...
sub make {
  ...
  return DecryptExtEntity->new($self->{_client}, $opts);
}
...
# Returns the current DecryptExt data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of DecryptExt fields).
sub match_get {
  my ($self) = @_;
...
# Create a new DecryptExt.
#
# reqdata: body data hashref (DecryptExtCreateData shape). ctrl: optional
# per-call control. Returns the created DecryptExt data (hashref); dies
