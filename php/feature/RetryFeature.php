...
// BluefinDecryptx SDK retry feature
...
class BluefinDecryptxRetryFeature extends BluefinDecryptxBaseFeature
{
    ...
    public function init(BluefinDecryptxContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (BluefinDecryptxContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(BluefinDecryptxContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(BluefinDecryptxContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
