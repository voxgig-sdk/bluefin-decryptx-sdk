...
class DecryptEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new DecryptEntity($this->_client, $opts);
    }

    /**
     * @param Decrypt|array $args Decrypt data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = BluefinDecryptxHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Decrypt|array The current Decrypt data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Decrypt fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = BluefinDecryptxHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Decrypt fields).
     */
     ...
     *
     * @param DecryptListMatch|array|null $reqmatch Match filter (any subset
     *   of Decrypt fields) as an assoc-array; DecryptListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Decrypt[]|array A list of Decrypt items as assoc-arrays at
     *   the SDK boundary; throws BluefinDecryptxError on failure (item-5 convention).
     */
     ...
     *
     * @param DecryptCreateData|array|null $reqdata Body data as an assoc-array;
     *   a typed DecryptCreateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Decrypt|array The created Decrypt as an assoc-array at the
     *   SDK boundary; throws BluefinDecryptxError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                if ($ctx->result->resdata) {
                    $this->_data = BluefinDecryptxHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
