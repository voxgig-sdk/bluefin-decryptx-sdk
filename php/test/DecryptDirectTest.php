...
class DecryptDirectTest extends TestCase
{
    public function test_direct_list_decrypt(): void
    {
        $setup = decrypt_direct_setup([
            ["id" => "direct01"],
            ["id" => "direct02"],
        ]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-list-decrypt", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
...
function decrypt_direct_setup($mockres)
{
    ...
    if ($live) {
        ...
        $client = new BluefinDecryptxSDK($merged_opts);
        return [
            "client" => $client,
            "calls" => $calls,
            "live" => true,
