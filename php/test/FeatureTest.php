...
// BluefinDecryptx SDK feature test
//
...
class FtCtrl extends BluefinDecryptxControl
{
...
class FtClient
{
    ...
    private BluefinDecryptxUtility $utility;

    public function __construct(BluefinDecryptxUtility $utility, array $options)
    {
    ...
    public function get_utility(): BluefinDecryptxUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (BluefinDecryptxContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public BluefinDecryptxContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (BluefinDecryptxContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = BluefinDecryptxConfig::make_feature($name);
    ...
    private static function buildUrl(BluefinDecryptxSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof BluefinDecryptxError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new BluefinDecryptxResponse($fetched);
        }
    ...
    private function fail(BluefinDecryptxContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (BluefinDecryptxContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
