// Generated direct-call test for the decrypt entity (mirrors the
...
static voxgig_value* decrypt_mock(void* ud, voxgig_value* args) {
  CALLS++;
...
static BluefinDecryptxSDK* decrypt_direct_setup(voxgig_value* mockres) {
...
int main(void) {
  ...
  // LIST
  {
    ...
    BluefinDecryptxSDK* sdk = decrypt_direct_setup(mockres);
    voxgig_value* params = v_map();
    PNError* err = NULL;
    voxgig_value* result = sdk_direct(sdk, cmap(3,
      "path", v_str("healthcheck"),
      "method", v_str("GET"),
      "params", params), &err);
