// BluefinDecryptxSDK client (generated — mirrors the rust Main fragment).
...
BluefinDecryptxSDK* bluefindecryptx_sdk_new(voxgig_value* options) {
  BluefinDecryptxSDK* sdk = (BluefinDecryptxSDK*)calloc(1, sizeof(BluefinDecryptxSDK));
...
BluefinDecryptxSDK* test_sdk(voxgig_value* testopts, voxgig_value* sdkopts) {
  ...
  BluefinDecryptxSDK* sdk = bluefindecryptx_sdk_new(sdkopts);
  free(sdk->mode);
  sdk->mode = strdup("test");
  return sdk;
}
