...
public class PipelineTest
{
    ...
    private static (BluefinDecryptxSDK, Utility) PlClient(Dictionary<string, object?>? sdkopts)
    {
        var client = BluefinDecryptxSDK.TestSDK(null, sdkopts);
        return (client, client.GetUtility());
    }

    private static Context PlCtx(BluefinDecryptxSDK client, Utility utility,
    ...
    private static string ErrCode(Action act)
    {
        ...
        catch (BluefinDecryptxError err)
        {
            return err.Code;
        }
        catch (Exception)
    ...
    private static Utility UtilWith(BluefinDecryptxSDK client, FetcherFunc fetcher)
    {
    ...
    public void MakeRequestTransportErrorCarriedOnResponse()
    {
        ...
        Assert.True(resp.Err is BluefinDecryptxError pe && pe.Code == "boom",
