// BluefinDecryptx SDK — drives the primary utility functions against the shared
...
static std::shared_ptr<BluefinDecryptxSDK> client() { return BluefinDecryptxSDK::testSDK(); }

// Helper: create basic test context.
static CtxPtr makeTestCtx(std::shared_ptr<BluefinDecryptxSDK> client_, UtilityPtr utility) {
  CtxSpec cs;
  cs.setOpname("load");
...
static CtxPtr makeTestFullCtx(std::shared_ptr<BluefinDecryptxSDK> client_, UtilityPtr utility) {
...
static void fetcherLive() {
  ...
  auto liveClient = std::make_shared<BluefinDecryptxSDK>(opts);
...
static void fetcherBlockedTestMode() {
  ...
  Value opts = fhMap({{"system", fhMap({{"fetch", Value(fetchFn)}})}});
  auto blockedClient = std::make_shared<BluefinDecryptxSDK>(opts);
...
static void makeSpecBasic() {
  ...
  auto specClient = BluefinDecryptxSDK::testSDK(Value::undef(), setupOpts);
...
static void prepareAuthBasic() {
  ...
  auto authClient = BluefinDecryptxSDK::testSDK(Value::undef(), setupOpts);
