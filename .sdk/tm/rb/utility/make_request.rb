...
module BluefinDecryptxUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = BluefinDecryptxResponse.new({})
    ...
    if fetch_err
      ...
      response = BluefinDecryptxResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = BluefinDecryptxResponse.new(fetched)
    else
