// BluefinDecryptx SDK test runner - shared infrastructure for the generated test
...
enum SdkRunner {
  ...
  // three dirs from this file (swift/Tests/BluefinDecryptxSDKTests -> project).
  static func testJsonPath(_ file: String = #filePath) -> String {
    let dir = URL(fileURLWithPath: file)
      .deletingLastPathComponent()  // BluefinDecryptxSDKTests
  ...
  static func errCode(_ e: Error?) -> String {
    (e as? BluefinDecryptxError)?.code ?? ""
  }
  ...
  static func makeCtxFromMap(_ ctxmapIn: VMap?, _ client: BluefinDecryptxSDK?, _ utility: Utility?)
    -> Context
  {
    ...
    if let resMap = ctxmap.entries["result"]?.asMap {
      ...
      if let errMap = resMap.entries["err"]?.asMap, let msg = errMap.entries["message"]?.asString {
        ctx.result!.err = BluefinDecryptxError("", msg, nil)
      }
    }
  ...
  static func fixCtx(_ ctx: Context, _ client: BluefinDecryptxSDK) {
  ...
  static func errFromMap(_ m: VMap?) -> Error? {
    ...
    return BluefinDecryptxError(code, msg, nil)
  }
}
