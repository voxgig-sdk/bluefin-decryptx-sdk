...
{
  my $sdk = BluefinDecryptxSDK->test({ 'net' => { 'offline' => 1 } }, undef);
...
{
  my $sdk = BluefinDecryptxSDK->test({ 'net' => { 'failTimes' => 1, 'failStatus' => 503 } }, undef);
  my $res = $sdk->direct({ 'path' => '/ping' });
...
{
  my $sdk = BluefinDecryptxSDK->test({ 'net' => { 'errorTimes' => 1 } }, undef);
  my $res = $sdk->direct({ 'path' => '/ping' });
...
{
  my $delay = 60;
  my $sdk = BluefinDecryptxSDK->test({ 'net' => { 'latency' => $delay } }, undef);
...
{
  my $sdk = BluefinDecryptxSDK->test(undef, undef);
