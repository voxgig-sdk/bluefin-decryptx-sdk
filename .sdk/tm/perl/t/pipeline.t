...
sub resp {
  ...
  return BluefinDecryptxResponse->new({
    'status' => $status,
...
sub spec_of {
  my (%map) = @_;
  return BluefinDecryptxSpec->new({
...
sub is_sdk_error {
  my ($err) = @_;
  return Scalar::Util::blessed($err) && $err->isa('BluefinDecryptxError');
}
...

{
  ...
  $ctx->{result} = BluefinDecryptxResult->new({});
...

{
  ...
  $ctx->{spec} = spec_of();
  $ctx->{result} = BluefinDecryptxResult->new({ 'ok' => 1, 'resdata' => [] });
  my ($result, $err) = $utility->{make_result}->($ctx);
...
{
  my $ctx = make_ctx();
  my $preset = BluefinDecryptxResult->new({ 'ok' => 1 });
...
{
  my $boom = BluefinDecryptxError->new('boom', 'boom');
...

{
  ...
  $u->{make_fetch_def} = sub {
    return (undef, BluefinDecryptxError->new('fetchdef_boom', 'boom'));
  };
  my $ctx = make_ctx(utility => $u);
  $ctx->{spec} = request_spec();
  my ($response, $err) = $u->{make_request}->($ctx);
...
{
  my $ctx = make_ctx();
  $ctx->{result} = BluefinDecryptxResult->new({ 'ok' => 1, 'resdata' => 42 });
  is($utility->{done}->($ctx), 42, 'done returns resdata on success');
}

{
  my $ctx = make_ctx();
  $ctx->{result} = BluefinDecryptxResult->new({ 'ok' => 0 });
...

{
  ...
  $ctx->{result} = BluefinDecryptxResult->new({ 'ok' => 1, 'resdata' => 7 });
...

{
  ...
  $ctx->{result} = BluefinDecryptxResult->new({ 'ok' => 0, 'resdata' => 'fallback' });
...
{
  my $oclient = BluefinDecryptxSDK->test(undef, undef);
  ...
  $oclient->{features} = [];
  my $a = BluefinDecryptxBaseFeature->new;
  my $b = BluefinDecryptxBaseFeature->new;
...
sub named_feature {
  my ($name) = @_;
  my $f = BluefinDecryptxBaseFeature->new;
  $f->{name} = $name;
  return $f;
}
...

{
  ...
  ok(BluefinDecryptxHelpers::is_true($o2->{feature}{metrics}{active}),
    'feature order: array normalized to a map (metrics opts preserved)');
  ok(BluefinDecryptxHelpers::is_true($o2->{feature}{test}{active}),
...
sub auth_ctx {
  ...
  my $ctx = BluefinDecryptxContext->new({
    'client' => PipelineOptClient->new($options),
    'utility' => $utility,
    'opname' => 'load',
  }, undef);
  $ctx->{spec} = defined $headers
    ? BluefinDecryptxSpec->new({ 'headers' => $headers, 'step' => 's' })
    : undef;
  return $ctx;
}
...
{
  my $ctx = make_ctx();
  $ctx->{response} = BluefinDecryptxResponse->new({ 'status' => 200 });
  $ctx->{result} = BluefinDecryptxResult->new({});
...
{
  my $ctx = make_ctx();
  $ctx->{response} = BluefinDecryptxResponse->new({
