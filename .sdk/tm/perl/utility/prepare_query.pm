# BluefinDecryptx SDK utility: prepare_query
...
$REGISTRY{prepare_query} = sub {
  ...
  my $params = [];
  if ($point) {
    my $p = BluefinDecryptxHelpers::gp($point, 'params');
  ...
  if ($items) {
    for my $item (@$items) {
      my ($key, $val) = @$item;
      next unless BluefinDecryptxHelpers::rb_truthy($val) && defined $key && !ref $key;
