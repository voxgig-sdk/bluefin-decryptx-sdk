# BluefinDecryptx SDK utility: prepare_auth
...
$REGISTRY{prepare_auth} = sub {
  ...
  if (!defined BluefinDecryptxHelpers::gp($options, 'auth')) {
  ...
  if (!defined $apikey || Voxgig::Struct::is_none($apikey)
    ...
    my $auth_prefix = BluefinDecryptxHelpers::gpath($options, 'auth.prefix');
