# BluefinDecryptx SDK utility: make_options
...
$REGISTRY{make_options} = sub {
  ...
  my $custom_utils = BluefinDecryptxHelpers::gp($options, 'utility');
  ...
  my @featureorder;
  my $feature_raw = BluefinDecryptxHelpers::gp($opts, 'feature');
  if (Voxgig::Struct::islist($feature_raw)) {
    my %fmap;
    for my $entry (@$feature_raw) {
      next unless Voxgig::Struct::ismap($entry);
      my $name = BluefinDecryptxHelpers::gp($entry, 'name');
  ...
  BluefinDecryptxHelpers::coerce_bools($opts);
  ...
  my $sys_fetch = BluefinDecryptxHelpers::gpath($opts, 'system.fetch');
  ...
  my $clean_keys = BluefinDecryptxHelpers::gpath($opts, 'clean.keys');
  ...
  if (!@featureorder) {
    my $fmap = BluefinDecryptxHelpers::to_map(
      BluefinDecryptxHelpers::gp($opts, 'feature')) || {};
