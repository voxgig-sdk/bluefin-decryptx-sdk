# BluefinDecryptx SDK test feature
...
package BluefinDecryptxTestFeature;
...
sub init {
  ...
  my $entity = BluefinDecryptxHelpers::gp($options, 'entity');
  ...
  my $test_fetcher = sub {
    ...
    my $op = $fctx->{op};
    my $entmap = BluefinDecryptxHelpers::gp($entity, $op->{entity});
    ...
    my $resolve_match = sub {
      ...
      for my $src ($fctx->{match}, $fctx->{data}) {
        ...
        my $v = BluefinDecryptxHelpers::gp($src, 'id');
    ...
    if ('load' eq $op->{name}) {
      ...
      my $ent = BluefinDecryptxHelpers::ge($found, 0);
      return $respond->(404, undef, { 'statusText' => 'Not found' })
        unless BluefinDecryptxHelpers::rb_truthy($ent);
      ...
      if (Voxgig::Struct::ismap($fctx->{reqdata})) {
        ...
        if ($op->{alias}) {
          my $alias_id = BluefinDecryptxHelpers::gp($op->{alias}, 'id');
      ...
      if (Voxgig::Struct::ismap($ent)) {
        my $id = BluefinDecryptxHelpers::gp($ent, 'id');
  ...
  my $net = BluefinDecryptxHelpers::gp($options, 'net');
...
sub make_netsim {
  ...
  my $do_sleep = sub {
    ...
    if (ref $net->{sleep} eq 'CODE') {
      ...
      BluefinDecryptxHelpers::sleep_ms($ms);
    }
  };
  ...
  return sub {
    ...
    if (BluefinDecryptxHelpers::is_true($net->{offline})) {
...
sub build_args {
  ...
  my $points = BluefinDecryptxHelpers::gpath($ctx->{config},
    "entity.$entname.op.$opname.points");
  my $point = BluefinDecryptxHelpers::ge($points, -1);

  my $params_path = BluefinDecryptxHelpers::gpath($point, 'args.params');
    ...
    if ($keys) {
      for my $key (@$keys) {
        ...
        if ($is_id || $is_reqd) {
          ...
          my $ka = $op->{alias} ? BluefinDecryptxHelpers::gp($op->{alias}, $key) : undef;
