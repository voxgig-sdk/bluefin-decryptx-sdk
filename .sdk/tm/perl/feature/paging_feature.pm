# BluefinDecryptx SDK paging feature
#
...
package BluefinDecryptxPagingFeature;
...
sub init {
  ...
  $self->{active} = BluefinDecryptxHelpers::is_true($self->{options}{active});
  return;
}
...
sub PreResult {
  ...
  if (Voxgig::Struct::ismap($body)) {
    my $bnext = BluefinDecryptxHelpers::gp($body, 'next');
    $paging->{next} = defined $paging->{next} ? $paging->{next} : $bnext
      if defined $bnext;
    my $bcursor = BluefinDecryptxHelpers::gp($body, 'cursor');
    $paging->{cursor} = $bcursor if defined $bcursor;
    my $bnextcursor = BluefinDecryptxHelpers::gp($body, 'nextCursor');
    $paging->{cursor} = $bnextcursor if defined $bnextcursor;
    my $bhasmore = $body->{hasMore};
    if (BluefinDecryptxHelpers::is_true($bhasmore) || BluefinDecryptxHelpers::is_false($bhasmore)) {
      $paging->{hasMore} = BluefinDecryptxHelpers::is_true($bhasmore) ? 1 : 0;
    }
  }
