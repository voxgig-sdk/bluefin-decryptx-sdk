# BluefinDecryptx SDK generic entity behaviour
#
...
defmodule BluefinDecryptx.EntityBase do
  alias Voxgig.Struct, as: S
  alias BluefinDecryptx.Helpers, as: H
  alias BluefinDecryptx.{Context, Utility, Pipeline}
  ...
  def construct(module, client, name, entopts \\ nil) do
    ...
    S.setprop(ent, "_utility", BluefinDecryptx.get_utility(client))
    S.setprop(ent, "_entopts", entopts)
    S.setprop(ent, "_data", S.jm([]))
    S.setprop(ent, "_match", S.jm([]))

    entctx =
      Context.new(S.jm(["entity", ent, "entopts", entopts]), BluefinDecryptx.get_root_ctx(client))
