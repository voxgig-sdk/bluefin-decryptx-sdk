...
defmodule BluefinDecryptx.PrimaryUtilityTest do
  use ExUnit.Case

  alias Voxgig.Struct, as: S
  alias BluefinDecryptx.Helpers, as: H
  alias BluefinDecryptx.{Context, Utility, Spec, Result}

  defp client, do: BluefinDecryptx.test()

  defp ctx(client, opname \\ "load") do
    Context.new(S.jm(["opname", opname]), BluefinDecryptx.get_root_ctx(client))
  end
  ...
  test "prepare_auth sets an authorization header when an apikey is present" do
    c = BluefinDecryptx.new(H.deep(%{"apikey" => "secret"}))
    ctx = ctx(c)
  ...
  test "make_options derives a clean key regex" do
    c = BluefinDecryptx.test()
    opts = BluefinDecryptx.options_map(c)
