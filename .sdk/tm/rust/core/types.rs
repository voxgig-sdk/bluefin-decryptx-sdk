...
/// BluefinDecryptxEntity: the full CRUD contract every generated entity
/// implements. Ops the API spec doesn't define are runtime-error stubs.
pub trait BluefinDecryptxEntity: Entity {
    fn load(&self, reqmatch: Value, ctrl: Value) -> Result<Value, BluefinDecryptxError>;
    ...
    fn update(&self, reqdata: Value, ctrl: Value) -> Result<Value, BluefinDecryptxError>;
    fn remove(&self, reqmatch: Value, ctrl: Value) -> Result<Value, BluefinDecryptxError>;
}

/// Transport: `(ctx, url, fetchdef) -> transport-shaped response map`.
/// An Ok(Value::Noval) mirrors go's (nil, nil) transport result.
pub type FetcherFn =
    Rc<dyn Fn(&Rc<Context>, &str, &Value) -> Result<Value, BluefinDecryptxError>>;
