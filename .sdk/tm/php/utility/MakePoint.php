...
class BluefinDecryptxMakePoint
{
    public static function call(BluefinDecryptxContext $ctx): array
    {
        if (isset($ctx->out['point'])) {
            ...
            if ($ctx->out['point'] instanceof BluefinDecryptxError) {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $point = $p;
                $select_def = BluefinDecryptxHelpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = BluefinDecryptxHelpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));
