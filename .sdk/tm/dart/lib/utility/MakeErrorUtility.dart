import '../BluefinDecryptxError.dart';
import '../Result.dart';
...
dynamic makeError(dynamic ctx, [dynamic err]) {
  ...
  final msg = 'BluefinDecryptxSDK: ' + opname.toString() + ': ' + errmsg(err);

  BluefinDecryptxError sdkerr;
  if (err is BluefinDecryptxError) {
    sdkerr = err;
    sdkerr.message = clean(ctx, msg);
  } else {
    final code = errcode(err);
    sdkerr = BluefinDecryptxError('' == code ? 'unknown' : code, clean(ctx, msg), ctx);
  }
