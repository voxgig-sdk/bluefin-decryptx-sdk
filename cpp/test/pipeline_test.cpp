// BluefinDecryptx SDK — direct unit tests for the operation-pipeline utilities
...
static std::shared_ptr<BluefinDecryptxSDK> plClient(const Value& sdkopts) {
  return BluefinDecryptxSDK::testSDK(Value::undef(), sdkopts);
}

static CtxPtr plCtx(std::shared_ptr<BluefinDecryptxSDK> client, UtilityPtr utility, const Value& ctrl) {
  CtxSpec cs;
  cs.setOpname("load");
...
static std::string names(std::shared_ptr<BluefinDecryptxSDK> client) {
  std::string out;
