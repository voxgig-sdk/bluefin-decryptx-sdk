// Command BluefinDecryptx-cli is a CLI + REPL for the BluefinDecryptx SDK,
...
const prompt = "bluefin-decryptx"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "decrypt decrypt_ext validation"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      BLUEFIN_DECRYPTX_APIKEY (api key), BLUEFIN_DECRYPTX_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: BLUEFIN_DECRYPTX_APIKEY carries the API key and
	// BLUEFIN_DECRYPTX_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("BLUEFIN_DECRYPTX_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("BLUEFIN_DECRYPTX_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewBluefinDecryptxSDK(opts)
