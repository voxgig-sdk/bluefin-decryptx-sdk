# bluefin-decryptx-cli

AQL-driven command-line client **and** interactive REPL for the BluefinDecryptx
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/bluefin-decryptx-cli)
make build

# 2. See usage (words, entities, env vars)
./bluefin-decryptx-cli --help
...
./bluefin-decryptx-cli list decrypt
./bluefin-decryptx-cli list decrypt_ext

# 5. Override the API base URL for a single call
BLUEFIN_DECRYPTX_BASE=https://api.example.com ./bluefin-decryptx-cli list decrypt

# 6. No arguments -> interactive REPL
./bluefin-decryptx-cli
bluefin-decryptx> list decrypt
bluefin-decryptx> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/bluefin-decryptx-cli
   ```
...
3. **Run a query.** Evaluate an AQL expression against the API (or run with no
   ...
   ```sh
   ./dist/*/bluefin-decryptx-cli list decrypt
   ```
...
Run with no arguments to open a REPL (prompt `bluefin-decryptx>`). Each line is
evaluated as its own AQL expression:

```text
$ ./bluefin-decryptx-cli
bluefin-decryptx> list decrypt
bluefin-decryptx> /help
bluefin-decryptx> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an AQL atom (e.g. `decrypt`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/bluefin-decryptx-cli`. |
...
(`./bluefin-decryptx-cli <expr>`) and interactively (the REPL), and expressions compose the
