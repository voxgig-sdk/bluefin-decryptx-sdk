// BluefinDecryptxError: the SDK error type (mirrors go core/error.go / the rust
...
pub const BluefinDecryptxError = struct {
    sdk: []const u8 = "BluefinDecryptx",
    code: []const u8,
    msg: []const u8,
    ...
    pub fn make(code: []const u8, msg: []const u8) *BluefinDecryptxError {
        const e = mem.a().create(BluefinDecryptxError) catch unreachable;
        e.* = .{ .sdk = "BluefinDecryptx", .code = code, .msg = msg };
        return e;
    }
};
