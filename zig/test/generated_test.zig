...
test "decrypt_list_smoke" {
    const fixture = h.jo(&.{.{ "decrypt", h.jo(&.{.{ "t01", h.jo(&.{.{ "id", h.vstr("t01") }}) }}) }});
    const testsdk = sdk.test_sdk(h.jo(&.{.{ "entity", fixture }}), vnull());
    const e = testsdk.decrypt(vnull());
    const res = e.list(vnull(), vnull());
    try std.testing.expect(res == .ok);
}

test "decrypt_stream_smoke" {
    ...
    const fixture = h.jo(&.{.{ "decrypt", h.jo(&.{
