// DecryptExt entity client (generated — mirrors the go/rust Entity fragment).
...
pub const DecryptExtEntity = struct {
    name: []const u8 = "decrypt_ext",
    client: *sdk.BluefinDecryptxSDK,
    utility: *Utility,
    entopts: Value,
    data: Value,
    mtch: Value,
    ...
    pub fn new(client: *sdk.BluefinDecryptxSDK, entopts_in: Value) *DecryptExtEntity {
        ...
        const e = h.A().create(DecryptExtEntity) catch unreachable;
        e.* = .{
            .name = "decrypt_ext",
            .client = client,
    ...
    fn ent_ctx(self: *DecryptExtEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *DecryptExtEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *DecryptExtEntity, ctx: *Context) OpResult {
        const v = self.utility.done(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn run_op(self: *DecryptExtEntity, ctx: *Context, post_done: *const fn (*DecryptExtEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *DecryptExtEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *DecryptExtEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return DecryptExtEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *DecryptExtEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *DecryptExtEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *DecryptExtEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *DecryptExtEntity, _reqmatch: Value, _ctrl: Value) OpResult {
        _ = _reqmatch;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("load", self.name) };
    }

    pub fn list(self: *DecryptExtEntity, _reqmatch: Value, _ctrl: Value) OpResult {
        _ = _reqmatch;
        _ = _ctrl;
    ...
    pub fn create(self: *DecryptExtEntity, reqdata: Value, ctrl: Value) OpResult {
    ...
    fn create_post_done(self: *DecryptExtEntity, ctx: *Context) void {
    ...
    pub fn update(self: *DecryptExtEntity, _reqdata: Value, _ctrl: Value) OpResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *DecryptExtEntity, _reqmatch: Value, _ctrl: Value) OpResult {
        _ = _reqmatch;
        _ = _ctrl;
