// Decrypt entity client (generated — mirrors the go/rust Entity fragment).
...
pub const DecryptEntity = struct {
    name: []const u8 = "decrypt",
    ...
    pub fn new(client: *sdk.BluefinDecryptxSDK, entopts_in: Value) *DecryptEntity {
        ...
        const e = h.A().create(DecryptEntity) catch unreachable;
        e.* = .{
            .name = "decrypt",
            .client = client,
    ...
    fn ent_ctx(self: *DecryptEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *DecryptEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *DecryptEntity, ctx: *Context) OpResult {
        const v = self.utility.done(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn run_op(self: *DecryptEntity, ctx: *Context, post_done: *const fn (*DecryptEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *DecryptEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *DecryptEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return DecryptEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *DecryptEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *DecryptEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *DecryptEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *DecryptEntity, _reqmatch: Value, _ctrl: Value) OpResult {
        _ = _reqmatch;
        _ = _ctrl;
    ...
    pub fn list(self: *DecryptEntity, reqmatch: Value, ctrl: Value) OpResult {
    ...
    fn list_post_done(self: *DecryptEntity, ctx: *Context) void {
    ...
    pub fn create(self: *DecryptEntity, reqdata: Value, ctrl: Value) OpResult {
        const ctx = self.utility.make_context(CtxSpec{
            .opname = "create",
            .ctrl = ctrl,
            .mtch = self.mtch,
    ...
    fn create_post_done(self: *DecryptEntity, ctx: *Context) void {
        if (ctx.result) |result| {
    ...
    pub fn update(self: *DecryptEntity, _reqdata: Value, _ctrl: Value) OpResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *DecryptEntity, _reqmatch: Value, _ctrl: Value) OpResult {
        _ = _reqmatch;
        _ = _ctrl;
