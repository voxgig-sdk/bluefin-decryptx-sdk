// Validation entity client (generated — mirrors the go/rust Entity fragment).
...
pub const ValidationEntity = struct {
    name: []const u8 = "validation",
    ...
    pub fn new(client: *sdk.BluefinDecryptxSDK, entopts_in: Value) *ValidationEntity {
        ...
        const e = h.A().create(ValidationEntity) catch unreachable;
        e.* = .{
            .name = "validation",
            .client = client,
    ...
    fn ent_ctx(self: *ValidationEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *ValidationEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *ValidationEntity, ctx: *Context) OpResult {
        const v = self.utility.done(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn run_op(self: *ValidationEntity, ctx: *Context, post_done: *const fn (*ValidationEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *ValidationEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *ValidationEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return ValidationEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *ValidationEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *ValidationEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *ValidationEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *ValidationEntity, _reqmatch: Value, _ctrl: Value) OpResult {
        _ = _reqmatch;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("load", self.name) };
    }

    pub fn list(self: *ValidationEntity, _reqmatch: Value, _ctrl: Value) OpResult {
        _ = _reqmatch;
        _ = _ctrl;
    ...
    pub fn create(self: *ValidationEntity, reqdata: Value, ctrl: Value) OpResult {
    ...
    fn create_post_done(self: *ValidationEntity, ctx: *Context) void {
    ...
    pub fn update(self: *ValidationEntity, _reqdata: Value, _ctrl: Value) OpResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *ValidationEntity, _reqmatch: Value, _ctrl: Value) OpResult {
        _ = _reqmatch;
        _ = _ctrl;
