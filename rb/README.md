# BluefinDecryptx Ruby SDK



The Ruby SDK for the BluefinDecryptx API — an entity-oriented client using idiomatic Ruby conventions.
...
raise a `BluefinDecryptxError` (a `StandardError` subclass), so wrap
...
```ruby
# list returns an Array of Decrypt records (raises on error).
decrypts = client.Decrypt.list
```
...
```
rb/
├── BluefinDecryptx_sdk.rb       -- Main SDK module
