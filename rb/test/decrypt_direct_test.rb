...
class DecryptDirectTest < Minitest::Test
  def test_direct_list_decrypt
    setup = decrypt_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-decrypt", setup[:live] ? "live" : "unit")
    if _should_skip
...
def decrypt_direct_setup(mockres)
  ...
  if live
    ...
    client = BluefinDecryptxSDK.new(merged_opts)
    return {
      client: client,
      calls: calls,
      live: true,
