...
class DecryptEntity
  ...
  def make
    opts = @_entopts.dup
    DecryptEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = BluefinDecryptxHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Decrypt, Hash] the current Decrypt data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = BluefinDecryptxHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Decrypt fields)
  def match_get
  ...
  #
  # @param reqmatch [DecryptListMatch, Hash, nil] match filter (any subset of
  #   Decrypt fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Decrypt>, Array] the matching Decrypt items; raises BluefinDecryptxError on failure
  ...
  #
  # @param reqdata [DecryptCreateData, Hash, nil] body data
  # @param ctrl [Object, nil] optional per-call control
  # @return [Decrypt, Hash] the created Decrypt; raises BluefinDecryptxError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        if ctx.result.resdata
          @_data = BluefinDecryptxHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
