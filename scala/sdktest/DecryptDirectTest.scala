// Generated direct-call tests for the decrypt entity (mirrors the java
...
object DecryptDirectTest {

  private class DirectSetup(val client: BluefinDecryptxSDK, val calls: JList[JMap[String, Object]])
  ...
  private def directSetup(mockres: Object): DirectSetup = {
    ...
    val client = new BluefinDecryptxSDK(SdkTestSupport.om(
  ...
  def run(rep: SdkTestReport): Unit = {
    rep.scope("direct-list-decrypt") {
      ...
      rep.eq("direct-list-decrypt.ok", java.lang.Boolean.TRUE, result.get("ok"))
      rep.eqI("direct-list-decrypt.status", 200, Helpers.toInt(result.get("status")))
      rep.check("direct-list-decrypt.islist", result.get("data").isInstanceOf[JList[?]], "expected data to be an array, got " + result.get("data"))
      val listData = result.get("data").asInstanceOf[JList[Object]]
      rep.eqI("direct-list-decrypt.size", 2, listData.size())
      rep.eqI("direct-list-decrypt.calls", 1, setup.calls.size())
    }
