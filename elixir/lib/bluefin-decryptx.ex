...
defmodule BluefinDecryptx do
  alias Voxgig.Struct, as: S
  alias BluefinDecryptx.Helpers, as: H
  alias BluefinDecryptx.{Utility, Context, Spec}
  ...
  def new(options \\ nil) do
    ...
    config = BluefinDecryptx.Config.make_config()
    ...
    if S.islist(feature_order) and S.size(feature_order) > 0 do
      Enum.each(0..(S.size(feature_order) - 1), fn i ->
        ...
        if fom != nil and S.getprop(fom, "active") == true do
          Utility.feature_add(rootctx, BluefinDecryptx.Features.make_feature(fname))
        end
      end)
    end
      ...
      _ ->
        ...
        cond do
          ...
          S.ismap(fetched) ->
            ...
            content_length =
              if S.ismap(headers), do: BluefinDecryptx.Feature.header_get(headers, "content-length"), else: nil
  ...
  @doc "Entity factory for decrypt."
  def decrypt(client, entopts \\ nil) do
    BluefinDecryptx.Entity.Decrypt.new(client, entopts)
  end

  @doc "Entity factory for decrypt_ext."
  def decrypt_ext(client, entopts \\ nil) do
    BluefinDecryptx.Entity.DecryptExt.new(client, entopts)
  end

  @doc "Entity factory for validation."
  def validation(client, entopts \\ nil) do
    BluefinDecryptx.Entity.Validation.new(client, entopts)
  end
