# Decrypt direct test (offline, mock system.fetch)

defmodule BluefinDecryptx.DecryptDirectTest do
  use ExUnit.Case
  ...
  defp mk(canned) do
    ...
    BluefinDecryptx.new(S.jm(["base", "http://localhost:8080", "system", S.jm(["fetch", fetch])]))
  end

  test "should direct list decrypt" do
    canned = S.jt([S.jm(["id", "direct01"]), S.jm(["id", "direct02"])])
    sdk = mk(canned)
    res = BluefinDecryptx.direct(sdk, H.deep(%{"path" => "/decrypt", "method" => "GET", "params" => %{}}))
