// decrypt direct API tests (generated from the API model).
...
public class DecryptDirectTest
{
    [Fact]
    public void DirectList()
    {
        var setup = DecryptDirectSetup(new List<object?>
        {
        ...
        var (_shouldSkip, _) = TestRunner.IsControlSkipped(
            "direct", "direct-list-decrypt", _mode);
        if (_shouldSkip)
        {
    ...
    private class DecryptDirectSetupResult
    {
    ...
    private static DecryptDirectSetupResult DecryptDirectSetup(object? mockres)
    {
        ...
        {
            ["BLUEFINDECRYPTX_TEST_DECRYPT_ENTID"] = new Dictionary<string, object?>(),
        ...
        if (live)
        {
            ...
            var entidRaw = env["BLUEFINDECRYPTX_TEST_DECRYPT_ENTID"];
            ...
            return new DecryptDirectSetupResult
            {
                Client = liveClient,
                Calls = calls,
                Live = true,
