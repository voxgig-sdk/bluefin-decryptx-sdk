...
from bluefindecryptx_types import (
    Decrypt,
    DecryptListMatch,
    DecryptCreateData,
)
...
class DecryptEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return DecryptEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Decrypt]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Decrypt().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def create(self, reqdata: DecryptCreateData, ctrl=None) -> Decrypt:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "create",
            "ctrl": ctrl,
