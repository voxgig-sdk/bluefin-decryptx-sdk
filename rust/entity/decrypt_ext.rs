// DecryptExt entity client (generated — mirrors the go Entity fragment).
...
use crate::core::error::BluefinDecryptxError;
use crate::core::helpers::{get_bool, getp, setp, to_map};
use crate::core::sdk::BluefinDecryptxSDK;
use crate::core::types::{Entity, BluefinDecryptxEntity};
use crate::core::utility_type::Utility;
use crate::utility::voxgigstruct as vs;
use crate::utility::voxgigstruct::Value;

pub struct DecryptExtEntity {
    name: String,
    #[allow(dead_code)]
...
impl DecryptExtEntity {
    pub fn new(client: &Rc<BluefinDecryptxSDK>, entopts: Value) -> Rc<DecryptExtEntity> {
        ...
        let e = Rc::new(DecryptExtEntity {
            name: "decrypt_ext".to_string(),
            client: client.clone(),
    ...
    ) -> Result<Value, BluefinDecryptxError> {
    ...
    pub fn stream(
        ...
        args: Value,
        callopts: Value,
    ) -> Result<std::vec::IntoIter<Value>, BluefinDecryptxError> {
...
impl Entity for DecryptExtEntity {
    ...
    fn make(&self) -> Rc<dyn Entity> {
        ...
        DecryptExtEntity::new(&self.client, opts) as Rc<dyn Entity>
    }
...
impl BluefinDecryptxEntity for DecryptExtEntity {
    fn load(&self, _reqmatch: Value, _ctrl: Value) -> Result<Value, BluefinDecryptxError> {
        Err(crate::core::helpers::unsupported_op("load", &self.name))
    }

    fn list(&self, _reqmatch: Value, _ctrl: Value) -> Result<Value, BluefinDecryptxError> {
        Err(crate::core::helpers::unsupported_op("list", &self.name))
    }


    fn create(&self, reqdata: Value, ctrl: Value) -> Result<Value, BluefinDecryptxError> {
        let ctx = self.utility.make_context(
            CtxSpec {
    ...
    fn update(&self, _reqdata: Value, _ctrl: Value) -> Result<Value, BluefinDecryptxError> {
        Err(crate::core::helpers::unsupported_op("update", &self.name))
    }

    fn remove(&self, _reqmatch: Value, _ctrl: Value) -> Result<Value, BluefinDecryptxError> {
