// Generated direct-call tests for the decrypt entity (mirrors the
...
use bluefin_decryptx_sdk::core::helpers::{getp, ja, jo, json_thunk, setp, to_int, to_map};
use bluefin_decryptx_sdk::utility::voxgigstruct as vs;
use bluefin_decryptx_sdk::{Value, BluefinDecryptxSDK};
...
fn decrypt_direct_setup(mockres: Value) -> DecryptDirectSetup {
    load_env_local();

    let calls: Rc<RefCell<Vec<Value>>> = Rc::new(RefCell::new(Vec::new()));

    let env = env_override(jo(vec![
        ("BLUEFINDECRYPTX_TEST_DECRYPT_ENTID", Value::empty_map()),
    ...
    if live {
        let client = BluefinDecryptxSDK::new(jo(vec![("apikey", getp(&env, "BLUEFINDECRYPTX_APIKEY"))]));
        let idmap = match to_map(&getp(&env, "BLUEFINDECRYPTX_TEST_DECRYPT_ENTID")) {
            Value::Map(m) => Value::Map(m),
            _ => Value::empty_map(),
        };
        return DecryptDirectSetup {
            client,
            calls,
            live: true,
            idmap,
...
#[test]
fn decrypt_direct_list() {
    let setup = decrypt_direct_setup(ja(vec![
        jo(vec![("id", Value::str("direct01"))]),
        jo(vec![("id", Value::str("direct02"))]),
    ]));
    let mode = if setup.live { "live" } else { "unit" };
    let (skip, reason) = is_control_skipped("direct", "direct-list-decrypt", mode);
    if skip {
        eprintln!(
            "skip: {}",
            if reason.is_empty() {
