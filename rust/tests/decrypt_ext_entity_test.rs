...
#[test]
fn decrypt_ext_entity_instance() {
    let testsdk = test_sdk(Value::Noval, Value::Noval);
    let ent = testsdk.decrypt_ext(Value::Noval);
    assert_eq!(ent.get_name(), "decrypt_ext");
}

#[test]
fn decrypt_ext_entity_stream() {
    ...
    let plainent = plainsdk.decrypt_ext(Value::Noval);
...
#[test]
fn decrypt_ext_entity_basic() {
    let setup = decrypt_ext_basic_setup(Value::Noval);
    // Per-op sdk-test-control.json skip — the basic test exercises a flow
    // with multiple ops; skipping any op skips the whole flow.
    let mode = if setup.live { "live" } else { "unit" };
    for op in ["create"] {
        let (skip, reason) = is_control_skipped("entityOp", &format!("decrypt_ext.{}", op), mode);
        if skip {
    ...
    if setup.synthetic_only {
        eprintln!("skip: live entity test uses synthetic IDs from fixture — set BLUEFINDECRYPTX_TEST_DECRYPT_EXT_ENTID JSON to run live");
        return;
    }
    let client = setup.client.clone();
    // CREATE
    let decrypt_ext_ref01_ent = client.decrypt_ext(Value::Noval);
    let decrypt_ext_ref01_data = to_map(&getp(
        &getpath(&["new", "decrypt_ext"], &setup.data),
        "decrypt_ext_ref01",
    ));

    let decrypt_ext_ref01_data_result = decrypt_ext_ref01_ent
        .create(decrypt_ext_ref01_data.clone(), Value::Noval)
        .expect("create failed");
    let decrypt_ext_ref01_data = to_map(&decrypt_ext_ref01_data_result);
    assert!(
        matches!(decrypt_ext_ref01_data, Value::Map(_)),
        "expected create result to be a map"
    );

}

fn decrypt_ext_basic_setup(extra: Value) -> EntityTestSetup {
    load_env_local();
    ...
    entity_data_file.push("decrypt_ext");
    entity_data_file.push("DecryptExtTestData.json");
    ...
    let idmap = vs::transform(
        &ja(vec![Value::str("decrypt_ext01"), Value::str("decrypt_ext02"), Value::str("decrypt_ext03")]),
        &jo(vec![(
            "`$PACK`",
            ja(vec![
                Value::str(""),
    ...
    let entid_env_raw = std::env::var("BLUEFINDECRYPTX_TEST_DECRYPT_EXT_ENTID").unwrap_or_default();
    let idmap_overridden =
        !entid_env_raw.trim().is_empty() && entid_env_raw.trim().starts_with('{');

    let env = env_override(jo(vec![
        ("BLUEFINDECRYPTX_TEST_DECRYPT_EXT_ENTID", idmap.clone()),
    ...
    let idmap_resolved = match to_map(&getp(&env, "BLUEFINDECRYPTX_TEST_DECRYPT_EXT_ENTID")) {
