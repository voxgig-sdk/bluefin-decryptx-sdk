...
class DecryptDirectTest {
  ...
  companion object {
    ...
    fun directSetup(mockres: Any?): DirectSetup {
      ...
      envm["BLUEFINDECRYPTX_TEST_DECRYPT_ENTID"] = linkedMapOf<String, Any?>()
      ...
      if (live) {
        ...
        mergedOpts["apikey"] = env["BLUEFINDECRYPTX_APIKEY"]
        setup.client = BluefinDecryptxSDK(mergedOpts)
        setup.live = true

        var idmap: MutableMap<String, Any?> = linkedMapOf()
        val entidRaw = env["BLUEFINDECRYPTX_TEST_DECRYPT_ENTID"]
      ...
      setup.client = BluefinDecryptxSDK(jm(
