...
public final class BluefinDecryptxSDK {
  ...
  // Idiomatic usage: try client.Decrypt().list(nil) or
  // try client.Decrypt().load(vm(("id", .string("..."))), nil).
  public func Decrypt(_ entopts: VMap? = nil) -> BluefinDecryptxEntityBase {
    return DecryptEntity(self, entopts)
  }

  // DecryptExt returns a DecryptExt entity bound to this client.
  // Idiomatic usage: try client.DecryptExt().list(nil) or
  // try client.DecryptExt().load(vm(("id", .string("..."))), nil).
  public func DecryptExt(_ entopts: VMap? = nil) -> BluefinDecryptxEntityBase {
    return DecryptExtEntity(self, entopts)
  }

  // Validation returns a Validation entity bound to this client.
  // Idiomatic usage: try client.Validation().list(nil) or
  // try client.Validation().load(vm(("id", .string("..."))), nil).
  public func Validation(_ entopts: VMap? = nil) -> BluefinDecryptxEntityBase {
    return ValidationEntity(self, entopts)
  }


  public static func testSDK(_ testoptsIn: VMap?, _ sdkoptsIn: VMap?) -> BluefinDecryptxSDK {
    ...
    let sdk = BluefinDecryptxSDK(sdkopts)
    sdk.mode = "test"
    return sdk
  }
}
