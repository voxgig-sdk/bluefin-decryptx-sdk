...
const SDK_NAME = 'BluefinDecryptxSDK'
...
const TEST_SEED = {"entity":{"decrypt":{"test01":{"id":"test01"}},"decrypt_ext":{"test01":{"id":"test01"}},"validation":{"test01":{"id":"test01"}}}}
...
function isRunnable(code: string): boolean {
  return (
    /new\s+BluefinDecryptxSDK\b/.test(code) ||
    /\bBluefinDecryptxSDK\.test\b/.test(code) ||
    /\bclient\s*\./.test(code)
  )
}
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+BluefinDecryptxSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/BluefinDecryptxSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
