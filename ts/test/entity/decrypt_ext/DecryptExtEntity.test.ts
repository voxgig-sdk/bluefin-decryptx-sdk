...
import { BluefinDecryptxSDK, BaseFeature, stdutil } from '../../..'
...
describe('DecryptExtEntity', async () => {
  ...
  // `test.live.delayMs`; only sleeps when BLUEFINDECRYPTX_TEST_LIVE=TRUE.
  afterEach(liveDelay('BLUEFINDECRYPTX_TEST_LIVE'))
  ...
  test('basic', async (t) => {
    ...
    const live = 'TRUE' === process.env.BLUEFIN_DECRYPTX_TEST_LIVE
    for (const op of ['create']) {
      if (maybeSkipControl(t, 'entityOp', 'decrypt_ext.' + op, live)) return
    }
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['BLUEFIN_DECRYPTX_TEST_DECRYPT_EXT_ENTID']
  ...
  const live = 'TRUE' === env.BLUEFIN_DECRYPTX_TEST_LIVE
