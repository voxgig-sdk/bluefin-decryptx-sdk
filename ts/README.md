# BluefinDecryptx TypeScript SDK



The TypeScript SDK for the BluefinDecryptx API — a type-safe, entity-oriented client with full async/await support.
...
`list()` resolves to an array of Decrypt objects — iterate it directly:
...
```ts
// Create — returns the created Decrypt
...
bluefin-decryptx/
├── src/
│   ├── BluefinDecryptxSDK.ts        # Main SDK class
