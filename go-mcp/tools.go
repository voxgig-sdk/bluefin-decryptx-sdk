...
type Args struct {
	Entity string         `json:"entity" jsonschema:"decrypt | decrypt_ext | validation"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.BluefinDecryptxSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "bluefin-decryptx_list",
		Description: "List records from BluefinDecryptx. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "bluefin-decryptx_load",
		Description: "Load a single record from BluefinDecryptx. " +
...
func runOp(client *sdk.BluefinDecryptxSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.BluefinDecryptxSDK, name string) (sdk.BluefinDecryptxEntity, error) {
	switch strings.ToLower(name) {
	case "decrypt":
		return client.Decrypt(nil), nil
	case "decrypt_ext":
		return client.DecryptExt(nil), nil
	case "validation":
		return client.Validation(nil), nil
