# bluefin-decryptx-mcp

[MCP](https://modelcontextprotocol.io) server exposing the BluefinDecryptx SDK as
two agent tools — `bluefin-decryptx_list` and `bluefin-decryptx_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/bluefin-decryptx-mcp)
make build

# 2. Provide credentials via the environment
export BLUEFIN_DECRYPTX_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user bluefin-decryptx \
  -- /absolute/path/to/bluefin-decryptx-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./bluefin-decryptx-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// bluefin-decryptx_list: first page of records
{ "entity": "decrypt" }
{ "entity": "decrypt", "query": { } }

// bluefin-decryptx_load: one record by id
{ "entity": "decrypt", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/bluefin-decryptx-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user bluefin-decryptx \
     -- "$PWD"/dist/*/bluefin-decryptx-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `bluefin-decryptx_list` and `bluefin-decryptx_load` tools now appear
   in new sessions. Ask the agent to *"list decrypt using bluefin-decryptx"*
   and it calls `bluefin-decryptx_list` with `{"entity":"decrypt"}`.
...
```sh
export BLUEFIN_DECRYPTX_APIKEY=sk_live_xxx            # API key
export BLUEFIN_DECRYPTX_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `bluefin-decryptx_list` tool
...
### Call the `bluefin-decryptx_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `bluefin-decryptx_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `bluefin-decryptx_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `BLUEFIN_DECRYPTX_APIKEY` | API key sent with every request. |
| `BLUEFIN_DECRYPTX_BASE` | Optional override of the API base URL. |

### Entities

The 3 entities valid as the `entity` argument:

decrypt | decrypt_ext | validation

### Smoke test via HTTP (raw JSON-RPC)

```sh
./bluefin-decryptx-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"bluefin-decryptx_load","arguments":{"entity":"decrypt","query":{"id":1}}}}'
```
